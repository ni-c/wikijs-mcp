 ...
 * and this server hands the model raw wiki pages, which are exactly that:
 * whatever somebody with edit rights typed. A random token that only ever
 * appears in a *previous* tool result cannot be guessed. The token is bound to a
 * resource key, so a confirmation for one target cannot be replayed for another.
 */
 ...
 *
 * Note what is NOT in here: no title, description or tag coming from the wiki.
 * Those are written by anyone with edit rights and this string is read by a
 * model. A page *path* may appear, because {@link identifier} proves it is one.
 */
