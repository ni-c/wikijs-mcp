...
export function buildToolFilter(config: {
  ...
  if (allow === undefined) {
    ...
    for (const entry of allow) {
      ...
      const matches = expand(entry, 'WIKIJS_ALLOW_TOOLS');
      if (matches.length === 0) {
        throw new ToolFilterError(
          `WIKIJS_ALLOW_TOOLS: no tool matches "${entry}". ` +
      ...
      if (survivors.length === 0) {
        if (entry.endsWith('*')) {
          ...
          console.error(
            `wikijs-mcp: WIKIJS_ALLOW_TOOLS: "${entry}" matches only write ` +
              'tools, which WIKIJS_READ_ONLY suppresses — it contributes nothing.'
          );
        ...
        throw new ToolFilterError(
          `WIKIJS_ALLOW_TOOLS: "${entry}" is a write tool, but WIKIJS_READ_ONLY ` +
            'is set — it is never registered. Remove it from WIKIJS_ALLOW_TOOLS, ' +
            `or unset WIKIJS_READ_ONLY. Available in read-only mode: ${[...READ_TOOLS].sort().join(', ')}.`
        );
      }
  ...
  for (const entry of deny ?? []) {
    ...
    const matches = expand(entry, 'WIKIJS_DENY_TOOLS');
    if (matches.length === 0) {
      throw new ToolFilterError(
        `WIKIJS_DENY_TOOLS: no tool matches "${entry}". Valid tools: ${catalogueList()}.`
      );
    }
  ...
  if (selected.size === 0) {
    throw new ToolFilterError(
      suppressedByReadOnly
        ? 'WIKIJS_ALLOW_TOOLS selects only write tools, but WIKIJS_READ_ONLY is ' +
            'set — the server would start with an empty tool list.'
        : 'WIKIJS_ALLOW_TOOLS/WIKIJS_DENY_TOOLS leave no tools registered — the ' +
