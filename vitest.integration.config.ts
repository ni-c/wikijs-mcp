 ...
 *
 * No coverage is collected here. What this suite proves is that Wiki.js
 * behaves as the code assumes, which is not a question about which lines ran.
 */
export default defineConfig({
  test: {
    include: ['test/integration/**/*.integration.test.ts'],
    // One story, shared state, one Wiki.js. Running the file's blocks against
    // each other in parallel would be a different test every time.
    fileParallelism: false,
    // Bootstrapping Wiki.js means a Postgres, a first boot, a setup wizard and
    // a master-process restart. On a cold CI runner that is minutes.
    hookTimeout: 600_000,
    testTimeout: 120_000,
  },
});
